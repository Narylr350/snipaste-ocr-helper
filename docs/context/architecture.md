...
- Settings: persist watch directory, tessdata directory override, OCR language, monitoring flag, and Start-with-Windows flag.
...
Snipaste is integrated only through its auto-save directory. OCR is isolated behind a provider interface; the MVP ships local Tesseract only and embeds `eng`/`chi_sim` tessdata into the single exe, extracting it to `%LOCALAPPDATA%\SnipasteOcrHelper\tessdata` on startup. Clipboard integration is isolated behind an adapter and overwrites text only after successful non-empty OCR output. Startup integration writes the current executable path to the current-user Run registry key when enabled.
...
- Embedded tessdata increases the published exe size.
