...
- OCR: `TesseractOCR` NuGet package, embedded `eng`/`chi_sim` tessdata, default language `eng+chi_sim`
...
- MVP scope: configure Snipaste auto-save directory, auto-extract embedded Tesseract tessdata, monitor new supported images, wait until files are stable, run local OCR, overwrite clipboard with non-empty recognized text, and provide tray settings/pause/resume/exit controls.
